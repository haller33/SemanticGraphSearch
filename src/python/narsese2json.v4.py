...
# ------------------------------------------------------------
# Tokenizer – a ordem dos padrões é CRÍTICA
# ------------------------------------------------------------
TOKEN_SPEC = [
    ('COMMENT',   r'//[^\n]*'),
    ('WHITESPACE',r'\s+'),
    ...
    ('BRACE',     r'\{[\w\s]+\}'),
    ('BRACKET',   r'\[[\w\s_]+\]'),
    ('RELATION',  r'-->|<->|==>|<=>|=/>|=/='),   # <-- operadores primeiro
    ('SPECIAL',   r'[<>=&|*(),.;?!%~-]'),        # <-- depois os caracteres isolados
]
...
class GraphEmitter:
    ...
    # ------------------------------------------------------------
    # Fallback: nó gigante e ligações para blocos internos
    # ------------------------------------------------------------
